...
int high_boost(char *input_path, char *output_path, float amp) {
    ...
    // Begin timestamp
    struct timespec begin_wall;
    clock_t begin_cpu;

    timespec_get(&begin_wall, TIME_UTC);
    begin_cpu = clock();
    ...
    clock_t end_cpu;
    timespec_get(&end_wall, TIME_UTC);
    end_cpu = clock();


    double time_wall = (end_wall.tv_nsec - begin_wall.tv_nsec) / 1000000000.0;
    double time_cpu = ((double) (end_cpu - begin_cpu)) / CLOCKS_PER_SEC;

    // (end_wall.tv_sec + begin_wall.tv_sec) -- 3531465734.000000
    // (end_wall.tv_nsec - begin_wall.tv_nsec) / 1000000000.0 -- 3531465682.000163

    printf("\nTempo wall: %.12lf segundos\n", time_wall);
    printf("Tempo cpu: %.12f segundos", time_cpu);
